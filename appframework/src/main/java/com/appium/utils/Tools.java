...
import com.appium.baselibs.MyException;
...
public class Tools {
  ...
  public static String getSource(WebDriver driver, By selector, String property) {
    ...
    return (String) ((JavascriptExecutor) driver).executeScript("return arguments[0]." + property
        + ";", element);
  }
  ...
  /**
   * If executeRow <0, only the non-empty execution row will be executed if executeRow>0, the
   * specified row when will be executed if executeRow=0, the whole data table will be returned.
   */
  public static String[][] getDataByTableName(String xlFilePath, String sheetName,
      String tableName, int executeRow) {
  ...
  /**
   * This function first gets the number of rows that execution(first column 0 is not empty) Then
   * create an array with the size of non empty row Then add the data to the new created array
   */
