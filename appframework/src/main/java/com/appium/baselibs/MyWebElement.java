...
public class MyWebElement implements WebElement {
	...
	private WebElement getWebElementByJquery(int index) {
		...
		we_Jquery = (WebElement) ((JavascriptExecutor) idriver)
	...
	public WebElement getWebElement() {
		...
		if (locator.contains("WebElement")) {
			we = webElement;
		} else if (locator.contains("jQuerySelector")) {
			we = getWebElementByJquery(index);
		} else if (locator.contains("class")) {
			// the index is zero based
			we = idriver.findElementsByClassName(locator).get(index);
		} else {
	...
	public int jqueryCountElementNumbers() {
		...
		String by = lArr[0];
		String using = locator.substring(by.length() + 1);
		Integer n = (Integer) ((JavascriptExecutor) idriver).executeScript("jQuery(\"" + using + "\").size()");
		return n;
	}
	...
	public WebElement findElement(By by, int id) {
		List<WebElement> webElements = findElements(by);
		webElement = webElements.get(id);
		return webElement;
	}
	...
	public void val(CharSequence... keysToSend) {
		webElement = getWebElement();
		print(" Type " + charSequence2String(keysToSend) + " in " + getElementName());
		((JavascriptExecutor) idriver).executeScript("arguments[0].value=arguments[1]", webElement, keysToSend);
	}
