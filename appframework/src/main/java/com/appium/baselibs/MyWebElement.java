...
import static com.appium.baselibs.PrintTestCases.charSequence2String;
...
public class MyWebElement implements WebElement {
  ...
  public MyWebElement(String locator) {
    idriver.manage().timeouts().implicitlyWait(IMPLICITLY_WAIT_TIME, TimeUnit.SECONDS);
  ...
  private WebElement getWebElementByJquery(int index) {
    ...
    we_Jquery =
        (WebElement) ((JavascriptExecutor) idriver).executeScript("return jQuery(\"" + using
            + "\").get(" + index + ")");
    return we_Jquery;
  }
    ...
    if (locator.contains("WebElement")) {
      ...
      if (locator.contains("jQuerySelector")) {
        ...
        if (index >= 0) {
          ...
          we = (WebElement) idriver.findElements(MyBy.locator(locator)).get(index);
        } else {
          we = idriver.findElement(MyBy.locator(locator));
        }
        if (subLocator != null) {
          if (sub_index == null) {
  ...
  /*
   * this method is used to count the total numbers of an elements group by jQuery
   */
  public int jqueryCountElementNumbers() {
    ...
    String using = locator.substring(by.length() + 1);
    Integer n =
        (Integer) ((JavascriptExecutor) idriver).executeScript("jQuery(\"" + using + "\").size()");
    return n;
  }
  ...
  /*
   * this method is similar to JQuery's val, which is used to insert data into a read-only textbox
   * which appended by data picker
   */
  public void val(CharSequence... keysToSend) {
    webElement = getWebElement();
    print(" Type " + charSequence2String(keysToSend) + " in " + getElementName());
    ((JavascriptExecutor) idriver).executeScript("arguments[0].value=arguments[1]", webElement,
        keysToSend);
  }
  ...
  public void val() {
    ...
    ((JavascriptExecutor) idriver).executeScript("$(webElement).val()", webElement);
  }
  ...
  public void verifyCssValue(String propertyName, String expectedValue) {
    ...
    MyVerifyAssert.verifyEquals(" Verify " + propertyName + " of " + elementName + " equals: "
  ...
  public void verifyAttributeContainsIgnoreSpaces(String attribute, String... expectedArr) {
    ...
    for (String s : expectedArr) {
      ...
      MyVerifyAssert.verifyContains(" Verify " + elementName + " contains(ignore space):"
          + "<br/>&nbsp; -----expected---: " + expected + "<br/>&nbsp; -----actual---: "
          + actualReplace, actualReplace, expectedReplace);
      expected = "";
    }
  }
  ...
  public void verifyMatches(String expected) {
    ...
    MyVerifyAssert.verifyMatches(idriver, "\nVerify " + elementName + " matches:" + "\nexpected:"
        + expected + "\n" + "actual:" + actual + "\n", actual, expected);
  }
  ...
  public void verifyContains(String... expectedArr) {
    ...
    for (String s : expectedArr) {
      ...
      MyVerifyAssert.verifyContains(" Verify " + elementName + " contains: "
          + "<br/>&nbsp; -----expected---: " + expected + "<br/>&nbsp; -----actual---: "
          + actualReplace, actualReplace, expected);
      expected = "";
    }
  }
  ...
  public void verifyContainsAttribute(String expected, String... attributeNames) {
    ...
    MyVerifyAssert.verifyContains(" Verify " + elementName + " equals: "
        + "<br/>&nbsp; -----expected---: " + expected + "<br/>&nbsp; -----actual---: " + actual,
        actual, expected);
  }
  ...
  public void verifyContainsIgnoreSpaces(String... expectedArr) {
    ...
    for (String s : expectedArr) {
      ...
      MyVerifyAssert.verifyContains(
          " Verify " + elementName + " contains(ignore spaces): "
              + "<br/>&nbsp; -----expected---: " + expected + "<br/>&nbsp; -----actual---: "
              + actual.replaceAll("\r|\n|\\s+", " "), actualReplace, expectedReplace);
      expected = "";
    }
  }
  ...
  public void verifyNotContainsIgnoreSpaces(String... expectedArr) {
    ...
    for (String s : expectedArr) {
      ...
      MyVerifyAssert.verifyNotContains(
          " Verify " + elementName + " NOT contains(ignore spaces): "
              + "<br/>&nbsp; -----expected---: " + expected + "<br/>&nbsp; -----actual---: "
              + actual.replaceAll("\r|\n|\\s+", " "), actualReplace, expectedReplace);
      expected = "";
    }
  ...
  public void verifySelected(boolean expected) {
    ...
    MyVerifyAssert.verifyEquals(" Verify " + elementName + " isSelected:"
        + "<br/>&nbsp; -----expected---: " + expected + "<br/>&nbsp; -----actual---: " + actual,
        actual, expected);
  }
  ...
  public void verifyEnabled(boolean expected) {
    ...
    MyVerifyAssert.verifyEquals(idriver, "\nVerify " + elementName + " isEnabled:" + "\nexpected:"
        + expected + "\n" + "actual:" + actual + "\n", actual, expected);
  }

  public void verifyDisplayed(boolean expected) {
    webElement = idriver.findElement(MyBy.locator(locator));
    String elementName = getElementName();
  ...
  public void verifyEquals(String expected, String... attributeNames) {
    ...
    MyVerifyAssert.verifyEquals(" Verify " + elementName + " equals: "
        + "<br/>&nbsp; -----expected---: " + expected + "<br/>&nbsp; -----actual---: " + actual,
        actual, expected);
  }
  ...
  public void verifySelectDropDownListSize(int expected) {
    ...
    MyVerifyAssert.verifyEquals(" Verify the total number in the select dropdown list: "
        + elementName + " equals to:" + "<br/>&nbsp; -----expected---: " + expected
  ...
  public void verifyFirstSelectedOption(String expected) {
    ...
    MyVerifyAssert.verifyEquals(" Verify First selected element options in Select box: "
        + elementName + " equals:" + "<br/>&nbsp; -----expected---: " + expected
        + "<br/>&nbsp; -----actual---: " + actual, actual, expected);
  }
  ...
  public void verifyAllSelectedOptions(String... expected) {
    ...
    // options selected
    MyVerifyAssert.verifyArrayEquals(" Verify All selected element options in Select box: "
        + elementName + " equals: " + "<br/>&nbsp; -----expected---: " + exp_sel_options.toArray()
        + "<br/>&nbsp; -----actual---: " + act_sel_options.toArray(), act_sel_options.toArray(),
        exp_sel_options.toArray());
  }
  ...
  public void verifyAllOptions(String[] expected) {
    ...
    // selected
    MyVerifyAssert.verifyArrayEquals(" Verify All visible element options in Select box: "
        + elementName + " equals: " + "<br/>&nbsp; -----expected---: " + exp_sel_options.toString()
        + "<br/>&nbsp; -----actual---: " + act_sel_options.toString(), act_sel_options.toArray(),
        exp_sel_options.toArray());
  }
  ...
  public void verifyOptionsByIndex(String expected, int index) {
    ...
    // Verify expected array for selected options match with actual options
    // selected
    MyVerifyAssert.verifyArrayEquals(" Verify NO." + index + " element options in Select box: "
        + elementName + " equals: " + "<br/>&nbsp; -----expected---: " + expected
        + "<br/>&nbsp; -----actual---: " + actual, actual, expected);
  }

  public MyWebElement waitUntilElementFound() {
    this.wait = new WebDriverWait(idriver, WEB_ELEMENT_WAIT_TIME);
    ...
    wait.until(new ExpectedCondition<WebElement>() {
      public WebElement apply(WebDriver driver) {
        ...
        if (element == null || !element.isDisplayed()) {
          /**
           * add element == null into judgement, because once login submit, signOut Link will be
           * 'null' at first so that the element.isDisplayed in waitUntilElementPresent method will
           * always throw java.null.lang.exception which cause cases wrong another workaround is
           * just to use waitUntilElementFound method, which works well every time..
           */
  ...
  public MyWebElement waitUntilElementPresentByTime(int t) {
    this.wait = new WebDriverWait(idriver, t);
    wait.until(new ExpectedCondition<WebElement>() {
      public WebElement apply(WebDriver driver) {
        WebElement element = getWebElement();
        if (element == null || !element.isDisplayed()) {
